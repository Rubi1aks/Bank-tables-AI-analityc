...
public class ForecastService {
    ...
    public Map<String, Object> generateForecast(String subject, String indicator, int horizon, String method) {
        ...
        if (result != null) {
            ...
            response.put("points", result.path("forecast"));
            response.put("qualityScore", result.path("metrics").path("RMSE").asDouble(0.12));
            return response;
        } else {
            Map<String, Object> fallback = new LinkedHashMap<>();
            fallback.put("error", "Python сервис недоступен");
            return fallback;
        }
    }
